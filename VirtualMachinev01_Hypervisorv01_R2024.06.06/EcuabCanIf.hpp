...
extern FUNC(void, CANIF_CODE) CanIf_DeInit(
   P2CONST(Type_CfgEcuabCanIf_st, AUTOMATIC, CANIF_APPL_CONST) ConfigPtr
);
...
extern FUNC(Type_EcuabCanIf_eStatusNotification, CANIF_CODE) CanIf_ReadRxNotifStatus(
   VAR(Type_SwcServiceCom_tIdPdu, AUTOMATIC) CanIfRxSduId
);
...
extern FUNC(void, CANIF_CODE) CanIf_ResetDynamicTxId(
...
extern FUNC(Std_ReturnType, CANIF_CODE) CanIf_Rn_ReadRxPduCanId(
