...
typedef P2FUNC(void, CANIF_APPL_MSG_CODE, Type_EcuabCanIf_FctConfirmationTx) (VAR(PduIdType, AUTOMATIC) CanTxPduId);
...
typedef P2FUNC(void, CANIF_APPL_STATE_CODE, Type_CfgEcuabCanIf_FctNotificationBusOff) (VAR(uint8, AUTOMATIC) ControllerId);
...
typedef uint8 EcuabCanIf_QueueTxFlagType;
# elif( CPU_TYPE == CPU_TYPE_16 )
typedef uint16 EcuabCanIf_QueueTxFlagType;
#else
typedef uint32 EcuabCanIf_QueueTxFlagType;
#endif

typedef boolean CanIf_CancelResultType;

typedef struct sEcuabCanIf_QueueTxBaseType{
...
#endif
  uint8         eSduLength;
}EcuabCanIf_QueueTxBaseType;

typedef struct sEcuabCanIf_QueueTxBaseWithDataType{
#if(CANIF_STATIC_FD_TXQUEUE == STD_OFF)
  uint8                   eSdu[CANIF_STATIC_BUFFER_SIZE];
#endif
  EcuabCanIf_QueueTxBaseType eBaseParams;
}EcuabCanIf_QueueTxBaseWithDataType;

typedef struct sCanIf_TxPrioByCanIdByteQueueType{
  EcuabCanIf_QueueTxBaseWithDataType eQueueParams;
  uint8                         eQueued;
...
typedef struct sEcuabCanIf_BufferTxPrioByIdCanBaseType{
  CanIf_TxBufferSizeType  eQueueCounter;
}EcuabCanIf_BufferTxPrioByIdCanBaseType;
